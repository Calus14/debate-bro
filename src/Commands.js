...
module.exports = {
    join: {
        ...
        async execute(bot, interaction) {
            ...
            if (!channel) return interaction.reply({ content: "⚠️ You aren't in a voice channel!", ephemeral: true });
            if (bot.sessions.has(interaction.guildId)) return interaction.reply({ content: "⚠️ Already recording!", ephemeral: true });
    ...
    leave: {
        ...
        async execute(bot, interaction) {
            ...
            if (!session) return interaction.reply({ content: "🚫 I'm not recording here!", ephemeral: true });

            const outPath = session.stop();
            bot.sessions.delete(interaction.guildId);
            await interaction.reply({ content: `✅ Recording stopped and saved: ${outPath}`, ephemeral: true });
        }
    }
};
